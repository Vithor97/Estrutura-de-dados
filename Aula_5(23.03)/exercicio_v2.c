...
void registroLog(int inicioOuFim, int fatorMultiplicativo, int TesteOuVetor){
	static const char *msg[] = {"Inicio","Fim"};
	FILE *pont_arq;
	pont_arq = fopen("LOG.txt", "a");
	time_t t = time(NULL);
    struct tm tm = *localtime(&t);
	...
	if(TesteOuVetor==0){
		fprintf(pont_arq, "\n%s do testes no vetor de %d00K: %d-%d-%d %d:%d:%d", msg[inicioOuFim], fatorMultiplicativo, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		printf("\n%s do testes no vetor de %d00K: %d-%d-%d %d:%d:%d", msg[inicioOuFim], fatorMultiplicativo, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	} else if(TesteOuVetor==1){
		fprintf(pont_arq, "\n%s dos testes: %d-%d-%d %d:%d:%d", msg[inicioOuFim], tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		printf("\n%s dos testes: %d-%d-%d %d:%d:%d", msg[inicioOuFim], tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	}
	fclose(pont_arq);
}
...
int main(){
	...
	FILE *pont_arq;
	pont_arq = fopen("LOG.txt", "a");
	printf("\nPrograma definido para %d teste(s)", qtdTest);
	fprintf(pont_arq,"\nPrograma definido para %d teste(s)\n", qtdTest);
	fclose(pont_arq);
	...
	registroLog(1,0,1);
	printf("\n\nUm arquivo chamada LOG.txt foi criado e todos os dados foram salvos nele!\n");
	return 0;
}
